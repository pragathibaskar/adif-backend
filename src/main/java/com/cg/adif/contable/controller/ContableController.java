...
public class ContableController {
	...
	private CodigoService codigosvc;
	 ...
	 @DeleteMapping("/codigo/{ts}")
	   void deleteCodigo(@PathVariable("ts") String ts)
	   {
       codigosvc.deleteById(ts);
	   }
	 ...
	 @PostMapping("/codigo")
		ResponseEntity<Provisionados> createNew(@RequestBody customProvionTo c)
		{
		 ...
		 pro.setCodigo_sap_expediente(c.getCodigo_sap_expediente());
		 pro.setCod_sociedad(c.getCod_sociedad());
		 pro.setPeriodo(date);
